...
def step_queued_video_job(run_id, event_store):
...
def step_wake_provisioner(run_id, video_helper):
    resp = httpx.post(f"http://localhost:{video_helper.agent_port}/", json={
        "run_id": run_id,
...
def step_check_real_vm_allocated(run_id, event_store, video_helper):
    success = False
    instance_id = None
    for _ in range(120):
...
def step_dispatch_job(run_id, video_helper, event_store):
    started = False
    for _ in range(30):
        effects = [e.effect for e in event_store.read_all(run_id)]
...
def step_trigger_deallocation(run_id, video_helper):
    resp = httpx.post(f"http://localhost:{video_helper.agent_port}/", json={
        "run_id": run_id,
        "notification_type": "instruction",
        "context": {}
...
def step_verify_deallocated(run_id, event_store, video_helper):
    deallocated = False
    for _ in range(30):
        effects = [e.effect for e in event_store.read_all(run_id)]
