...
def audit_file(file_path, docs_context, api_key):
    ...
    system_prompt = (
        ...
        "- **Inline Simulator Capabilities**: BDD Integration / Capacity Test Launchers are explicitly permitted to use inline simulator capabilities that subclass `AbstractCapability` and override/wrap tool execution (such as `wrap_tool_execute` to intercept bash commands, curls, or ssh calls like in `GenericAudioSimulator` or `GenericVideoSimulator`). This is a standard and fully permitted inline simulation pattern for BDD/Capacity tests, and does NOT violate the boundary mocking rules for Category 2.\n"
