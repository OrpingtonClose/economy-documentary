"""Production entry-point for the LTX-Video worker.

Shape:

1. Resolve config from env vars (worker id, playground url,
   infra-agent bump url, advertised endpoint url, advertised VRAM).
2. Build the video engine. In production this imports the real LTX
   backend lazily (to avoid heavy CUDA imports in unit tests). For
   tests :func:`main`'s engine is injectable via :func:`build_app`.
3. Self-register with the playground registry (no voice pinning for
   the video role).
...
process. See ``scripts/ltx_video_worker_bootstrap.sh`` for how the two
are supervised.
"""
...
from .engine import StubVideoEngine, VideoEngine
...
#: Distinct from the Qwen3-TTS port (29231) so both workers can in
#: theory coexist on the same VM (not used today, but cheap to
#: preserve).
WORKER_PORT: int = 29232
...
def _real_video_engine_factory() -> VideoEngine:
    """Default factory for the production LTX-Video engine.

    The real engine wraps ``diffusers`` + ``torch`` and is heavy to
    import; we defer that until the VM is actually booting. Tests
    override this factory with :class:`StubVideoEngine`. Missing
    backend falls back to the stub so a dry-run bootstrap on a VM
    without the weights still yields a functional FastAPI surface for
    the smoke test.
    """
    try:
        from ._ltx_engine import LTXVideoEngine  # noqa: PLC0415
    except ImportError:
        logger.warning(
            "ltx-video backend not installed, falling back to stub engine "
...
def heartbeat_loop(
    *,
    ...
    doesn't kill the worker. Also swallows any telemetry probe
    exception — a disk read error must not terminate the thread and
    let the registry silently mark this VM stale.
    """
    next_deadline = clock()
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="ltx-video-worker",
        description="LTX-Video worker FastAPI for Vast.ai VMs.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if cfg.playground_base_url:
        ...
        logger.warning(
            "no PLAYGROUND_BACKEND_URL set, worker runs unregistered (dev mode)"
        )
    ...
    if registry_client is not None:
        heartbeat_thread = threading.Thread(
            ...
            name="ltx-video-worker-heartbeat",
            daemon=True,
        )
        heartbeat_thread.start()

    engine = _real_video_engine_factory()
