...
def validate_media_files(
    ...
    """Validate that media files exist, are non-empty, and are playable.

    Uses ffprobe to check each file's duration and codec. Useful for
    agents to verify that their generated audio/video clips are valid
    before adding them to the OTIO timeline.

    Args:
        file_paths: Comma-separated list of file paths to validate.

    Returns:
        JSON string with per-file validation results.
    """
    import subprocess
    ...
    for path in paths:
        result: dict = {"path": path, "valid": False}
        ...
        try:
            ...
            if probe.returncode == 0:
                ...
                result["format"] = fmt.get("format_name", "unknown")
                streams = info.get("streams", [])
                result["codecs"] = [
                    {"type": s.get("codec_type"), "name": s.get("codec_name")}
                    for s in streams
                ]
                ...
                result["error"] = f"ffprobe failed (rc={probe.returncode})"
                all_valid = False
