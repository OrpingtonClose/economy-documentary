"""Playground worker-registry client.

The worker self-registers with ``POST /playground/workers`` on boot,
heartbeats periodically, and pins its voice. On clean shutdown it
unregisters itself. The infra agent also holds a separate
:class:`strands_agents.infra_agent.registry_client.PlaygroundRegistryClient`
that only knows how to ``DELETE`` the worker id — that one is the
fallback used when the guardian reaps the VM out from under us.
...
class RegistryRegisterError(RegistryClientError):
    """Raised on registration failure (non-2xx, including voice conflict)."""
...
class RegistryVoicePinError(RegistryClientError):
    """Raised on voice pin failure (409 on conflict, 404 on missing worker)."""
...
class PlaygroundRegistryClient:
    ...
    def register(
        ...
        *,
        ...
        logger.info(
            "worker_id=<%s>, role=<%s>, vram_gb=<%d>, voice_id=<%s>, url=<%s> | registering",
            worker_id,
            role,
            vram_gb,
            voice_id,
    ...
    def pin_voice(self, *, worker_id: str, voice_id: str) -> dict:
        """Pin ``voice_id`` to ``worker_id``. 409 raises, idempotent on the same pair."""
        url = (
            f"{self.base_url.rstrip('/')}/playground/workers/"
            f"{worker_id}/voice"
        )
        response = self._post_json(url, {"voice_id": voice_id})
        if 200 <= response.status_code < 300:
            return _decode_json(response.text)
        raise RegistryVoicePinError(
            f"pin_voice failed status={response.status_code} body={response.text[:500]!r}"
        )
